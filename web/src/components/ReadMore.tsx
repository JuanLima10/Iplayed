  ...
  if(children && children.toLocaleString().length > 400){
    return (
      <div className="cursor-pointer" onClick={() => setRead(!read)}>
        {read ?
          <p className="font-normal text-sm text-white-100 responsive:text-xs">{children}</p> :
          <div className="flex flex-col font-normal text-sm text-white-100 responsive:text-xs">
