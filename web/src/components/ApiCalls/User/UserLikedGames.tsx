...
export async function UserLikedGames(props: UserInfoGameProps) {
  const { data, count } = await api.get(`/user/like/${props.userId}`, {
    params: {
  ...
  return (
    <div className={`w-full h-32 flex items-center justify-center ${!props.isList && "border border-dotted"} border-white-100 bg-transparent rounded-lg m-1`}>
      <span className="font-bold text-lg text-center text-white-100 p-2 responsive:text-sm">There are no games in Likes yet! ❤️</span>
    </div>
  )
}

export function UserLikedGamesSkeleton({ limit }: { limit: number }) {
  return (
    [...Array(limit)].map((likes, index) => (
